...
MIN_TRANSCRIPT_FRAC=0.02 # Minimum % of transcripts that a tag must
                         # apply to for it to be reported
...
transcript_frac_per_tag_1 = normalize(transcripts_per_tag_1)
transcript_frac_per_tag_2 = normalize(transcripts_per_tag_2)

significant_tags = Set.new(transcript_frac_per_tag_1.keys).union(Set.new(transcript_frac_per_tag_2.keys)).select { |k| (transcript_frac_per_tag_1[k] || 0) >= MIN_TRANSCRIPT_FRAC || (transcript_frac_per_tag_2[k] || 0) >= MIN_TRANSCRIPT_FRAC }
...
foldChanges.keep_if {|k, v| significant_tags.include?(k) }
