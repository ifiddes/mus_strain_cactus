...
streambuf* FIOStream::openStreamBuf(const string& fileName,
    ...
    if (strBuf == NULL) {
        throw IOException(string("can't open \"") + fileName + "\" for " + ((ioMode & ios::out) ? "write" : "read") + " access");
    }
    return strBuf;
}
...
void FIOStream::close() {
    ...
    if (bad()) {
        throw IOException("I/O error on " + getFileName());
    }
}
